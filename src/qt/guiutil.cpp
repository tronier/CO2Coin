...
{
    // Convert co2coin:// to co2coin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("co2coin://"))
    {
        uri.replace(0, 12, "co2coin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CO2COIN.lnk";
}
...
{
    return GetAutostartDir() / "co2coin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("CO2COIN-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  co2coin-qt [" + tr("command-line options") + "]                     " + "\n";
